...
import { UserPermission, UserPermissionService } from '../user-permission';
...
export class CameraDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.userPermissionService.query()
            .subscribe((res: ResponseWrapper) => { this.userpermissions = res.json; }, (res: ResponseWrapper) => this.onError(res.json));
    }
    ...
    trackUserPermissionById(index: number, item: UserPermission) {
        return item.id;
    }
